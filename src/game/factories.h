...
struct factories {
	 ...
	 */
	static ecsTypes::entity createBall(const std::shared_ptr<ecsTypes::registry>& registry, glm::vec2 position, float rot, float radius, float mass, float restitution, bool isStatic = false);
	 ...
	 */
	static ecsTypes::entity createBox(const std::shared_ptr<ecsTypes::registry>& registry, glm::vec2 pos, float rot, float w, float h, float mass, float restitution, bool isStatic = false);
};